...
pon_sec_df$cargo_sec <- 'Secretario'
...
nodes_pon_sec <- data.frame(
  ...
  data.frame(cargo=unlist(pon_sec_df[, c("cargo_pon","cargo_sec")], use.names = FALSE)))

nodes_pon_sec <- nodes_pon_sec[!duplicated(nodes_pon_sec), ]
nodes_pon_sec <- nodes_pon_sec %>% 
  group_by(label) %>% 
  mutate(cargo = if(n() == 1) cargo else "Secretario/Ponente") %>% ungroup()
...
edges_pon_sec <- pon_sec_df %>% select(id_ponente_socio_rep,id_secretario_socio,instancia,epoca,materia) %>% 
  left_join(nodes_pon_sec, by = c("id_ponente_socio_rep" = "label")) %>% 
  left_join(nodes_pon_sec, by = c("id_secretario_socio" = "label")) %>%
...
primera_reit_df$cargo_pon <- 'Ponente'
...
primera_reit_df$cargo_sec <- 'Secretario'
...
nodes_1a_reit <- data.frame(
  ...
  data.frame(cargo=unlist(primera_reit_df[, c("cargo_pon","cargo_sec")], use.names = FALSE)))

nodes_1a_reit <- nodes_1a_reit[!duplicated(nodes_1a_reit), ]
nodes_1a_reit <- nodes_1a_reit %>% 
  group_by(label) %>% 
  mutate(cargo = if(n() == 1) cargo else "Secretario/Ponente")
...
edges_1a_reit <- primera_reit_df[, c("id_ponente_socio_rep","id_secretario_socio","instancia","epoca","materia")] %>% 
  left_join(nodes_1a_reit, by = c("id_ponente_socio_rep" = "label")) %>% 
  left_join(nodes_1a_reit, by = c("id_secretario_socio" = "label")) %>%
  rename(from=id.x, to=id.y) %>%
  select(from,to,instancia,epoca,materia)
...
ggraph(layout_pon_sec) +
  ...
  geom_node_point(aes(size=central_eigen, colour=central_eigen, shape=cargo)) +
  geom_node_text(aes(label=label), repel=T, size=2, nudge_y = 0.1, nudge_x = -0.1) +
  ggtitle('Pon-Sec') +
  theme_graph(base_family="sans") +
  labs(colour="Centralidad", size="Centralidad", shape="Cargo") 
