...
```{r}
# Segundo modelo:  Clasificación de materias considerando sólo tesis con materia única, y sólo datos 
# Coeficientes más relevantes para materia Administrativa:
ratios_2 %>%
  arrange(-Administrativa) %>%
  select(Administrativa) %>%
...
ggplot(clas_materia_unicas_socio) + 
  geom_point(aes(MeanDecreaseGini,Variable),color="blue",size=3) +
  scale_x_continuous(labels = scales::comma) +
  ggtitle('Clasificación de Materia (sólo variables sociodemográficas)') +
  xlab("Importancia (Reducción Índice GINI)") +
  ylab("Variable") +
  theme_bw() +
  theme(plot.title=element_text(hjust=0.5, vjust=0.5))
...
ggplot(clas_instancia_socio) + 
  geom_point(aes(MeanDecreaseGini,Variable),color="blue",size=3) +
  scale_x_continuous(labels = scales::comma) +
  ggtitle('Clasificación de Instancia (sólo variables sociodemográficas)') +
  xlab("Importancia (Reducción Índice GINI)") +
  ylab("Variable") +
  theme_bw() +
  theme(plot.title=element_text(hjust=0.5, vjust=0.5))
...
Realizamos regresión logísticas multinomial:
